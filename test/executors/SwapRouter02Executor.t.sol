...
contract SwapRouter02ExecutorTest is Test, PermitSignature, GasSnapshot, DeployPermit2 {
    ...
    function testExecuteWithJsScript() public {
        DutchOrder memory order = DutchOrder({
        info : OrderInfoBuilder.init(address(reactor)).withSwapper(swapper).withDeadline(block.timestamp + 100),
        decayStartTime : block.timestamp - 100,
        decayEndTime : block.timestamp + 100,
        ...
        runJsInputs[0] = "node";
        runJsInputs[1] = "test/js-scripts/signDutchOrder.js";

        // private key
        runJsInputs[2] = vm.toString(bytes32(swapperPrivateKey));
        //order info
        runJsInputs[3] = string(
            abi.encodePacked(
                vm.toString(address(order.info.reactor)),
                ",",
                vm.toString(order.info.swapper),
                ",",
                vm.toString(order.info.nonce),
                ",",
                vm.toString(order.info.deadline),
                ",",
                vm.toString(address(order.info.additionalValidationContract)),
                ",",
                vm.toString(bytes32(order.info.additionalValidationData))
            )
        );
        runJsInputs[4] = vm.toString(order.decayStartTime);
        runJsInputs[5] = vm.toString(order.decayEndTime);
        // input
        runJsInputs[6] = string(
            abi.encodePacked(
                vm.toString(address(order.input.token)),
                ",",
                vm.toString(order.input.startAmount),
                ",",
                vm.toString(order.input.endAmount)
            )
        );
        // outputs
        runJsInputs[7] = string(
            abi.encodePacked(
                vm.toString(order.outputs[0].token),
                ",",
                vm.toString(order.outputs[0].startAmount),
                ",",
                vm.toString(order.outputs[0].endAmount),
                ",",
                vm.toString(order.outputs[0].recipient)
            )
        );
        // permit contract
        runJsInputs[8] = vm.toString(address(permit2));
        // reactor contract
        runJsInputs[9] = vm.toString(address(reactor));

        bytes memory jsResult = vm.ffi(runJsInputs);

        (bytes memory encodeOrder, bytes memory signature) = abi.decode(jsResult,(bytes,bytes));

        snapStart("SwapRouter02ExecutorExecute");
        swapRouter02Executor.execute(
            SignedOrder(encodeOrder, signature),
            abi.encode(tokensToApproveForSwapRouter02, tokensToApproveForReactor, multicallData)
        );
        snapEnd();
